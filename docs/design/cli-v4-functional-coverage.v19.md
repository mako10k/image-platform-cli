# CLI Native API V4 functional coverage matrix, revision 19
...
- Status: functional implementation coverage complete; broader cutover validation pending
...
| `batch run`, `batch iterate` | Start a bounded campaign from a BatchPlan | cost bound, partial-result policy, threshold, rounds and wait | `GET /v4/batch-plans/{plan_id}`; `GET /v4/evaluation-rubrics`; `POST /v4/campaigns` | `V4-bound` |
| `batch status`, `batch evaluate`, `batch results` | Inspect campaign state, evaluation and outputs | collection and result projections | `GET /v4/campaigns/{campaign_id}`; `GET /v4/jobs/{job_id}` (Artifact IDs, no implicit download) | `V4-bound` |
| `batch cancel` | Cancel a campaign | cancellation state | `POST /v4/campaigns/{campaign_id}/cancel` | `V4-bound` |
...
| `edit composite` | Place and blend one image over another | affine transform, opacity, mask, crop and blend mode | `POST /v4/image-operations` | `V4-bound` |
| `edit run`, `edit verify` | Execute a deterministic edit program and verify reproducibility | input/mask bindings and repeated receipt/hash comparison | `POST /v4/image-operations` | `V4-bound` |
| `edit replace-object`, `edit replace-background` | Replace selected object or background coverage | threshold, invert/combine, morphology, padding and feather controls | `POST /v4/image-operations` | `V4-bound` |
...
| `edit raster adjust` | Apply ordered color and tone adjustments | hue, saturation, temperature, tint, exposure, brightness and contrast | `POST /v4/image-operations` | `V4-bound` |
...
| `edit raster auto-crop` | Crop from mask coverage | threshold and padding | `POST /v4/image-operations` | `V4-bound` |
...
| `edit raster resize`, `edit raster flip`, `edit raster rotate`, `edit raster canvas` | Apply deterministic affine geometry | fit, axis, quarter rotation, canvas placement and background | `POST /v4/image-operations` | `V4-bound` |
| `edit raster project-quad` | Project a texture onto a quadrilateral | destination points, four composite modes and offline preview | `POST /v4/image-operations` | `V4-bound` |
| `edit raster mesh` | Render a bounded mesh specification | mesh topology and texture input | `POST /v4/image-operations` | `V4-bound` |
...
remains bound to `image1`; its binding change requires separate owner authorization.
...
Quadrilateral projection is V4-bound. Four real local CPU fixtures cover source_over, replace,
multiply and screen. Focused tests cover request/texture binding, output geometry, malformed
evidence, convexity and coordinate rejection, reverse winding and offline preview. 35 tests passed
across projection, color matching and CLI composition; changed-file Ruff and strict Mypy passed.
No full-suite or package rerun was performed. Ten capability rows remain missing, broader cutover
validation remains pending, and image continues to delegate to image1.

## Revision 14 evidence

Composition is V4-bound. Five real local CPU fixtures cover four blend modes and a combined
translated, masked, half-opacity, cropped result. The existing V4 verifier now checks every command
in order within the single physical step, including each normalized command hash and planner
command list. Named inputs bind background/source, overlay and optional mask. Focused tests:
82 passed across composition, conversion, crop, color matching and CLI composition. Changed-file
Ruff and strict Mypy passed. No full-suite or package rerun was performed. Nine capability rows
remain missing; broader cutover validation is pending and image continues to delegate to image1.

## Revision 15 evidence

Affine geometry is V4-bound: resize/stretch/fit, both flips, 90/180/270 rotation and canvas
placement/background are implemented. Pure geometry helpers moved unchanged from V1 into common.
Eight real local CPU fixtures match the API normalized contract. Focused geometry/crop/V1 tests:
26 passed, 29 unrelated cases deselected. Changed-source Mypy and Ruff passed. No full suite or
package rerun. Eight capability rows remain missing; image remains image1.

## Revision 16 evidence

Generic run and verify are V4-bound. Local binding/loading moved into common; V4 normalization
uses the exported public DeterministicEditProgram schema from API commit 5d053dd. Existing normalized
fixtures remain unchanged. Focused program/composite/V1 checks: 87 passed, 57 deselected; after the
type fix and adding the split-receipt case, the program file passed 53 cases. Eight changed source
files passed strict Mypy; Ruff passed. Split-response tests are derived contract cases: only the
final execution receipt is returned, so intermediate source bytes/pixels cannot be independently
verified and are not claimed as verified. Logical/physical program identities, order, static input
hashes and final output are checked. No full-suite or package rerun. Seven rows remain missing.

## Revision 17 evidence

Ordered color/tone adjustments, mask auto-crop and mesh rendering are V4-bound. Pure raster
program builders moved from V1 to common. Seven real local CPU API fixtures cover adjustment
order, white balance, mask padding and a textured mesh. Focused raster/program/V1 checks: 86
passed, 19 deselected. Changed-source Ruff and strict Mypy passed. No full-suite or package
rerun. Four capability rows remain missing; image continues to delegate to image1.

## Revision 18 evidence

Object/background replacement is V4-bound. Shared pure builders retain mask union, intersection,
subtraction, threshold, inversion, morphology, padding and feather ordering. Nonfinite decimal
controls now fail locally in both versions. Eight real local CPU fixtures cover the variants;
object/background pixel checks confirm selected replacement and preserved base regions. Focused
replacement/program/V1 tests: 96 passed, 20 deselected. Five changed source files passed strict
Mypy and Ruff. No full-suite or package rerun. Three campaign rows remain for verification.

## Revision 19 evidence

The three campaign rows are V4-bound. Existing V4 code is verified for bounded creation/polling,
discovered iteration policy, state, pending/completed evaluation, partial-result Job/Artifact
projection and cancellation. Focused campaign/Job/Artifact/V1 checks: 53 passed, 46 deselected.
The matrix now accurately describes results as metadata: V1 campaign_results also returns Job and
Artifact IDs without downloading bytes. Explicit artifact download remains covered by its own row.
No new campaign product behavior was necessary, and no full-suite or package rerun was performed.
All functional rows are covered; overall cutover validation and owner-authorized switching remain.
